...
`docker build --squash -t henrij22/ikarus-dev:latest . &> build.log`
`docker build --squash -t henrij22/ikarus:latest . &> build.log`
...
`docker build --build-arg branch=<branch-name> --squash -t henrij22/<branch-name-must-be-lowercase> . &> build.log`
